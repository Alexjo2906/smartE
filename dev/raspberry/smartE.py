###################    Control SmartE from Pi activation of push button    ###################
...
while True: 
    try:
        ...
        GPIO.output(redstsLed, 0) # Indicate SmartE has passed the arduino tests 
        sleep(3) # Sleep to indicate the result from above

        GPIO.output(redstsLed, 1) # Indicate SmartE is still running tests
        delay = 1 # Controls how fast it goes
        steps = 1000 # Controls the revolutions
        forward(int(delay) / 1000.0, int(steps))
        GPIO.output(redstsLed, 0) # Indicate SmartE has passed the mech test
        sleep(3) # Sleep to indicate the result from above

        GPIO.output(redstsLed, 1) # Indicate SmartE is still running tests
        tryRelay(pinValvAgua)
        GPIO.output(redstsLed, 0) # Indicate SmartE has passed the first water test 
        ...
        sleep(1.5) # Sleep to indicate the result from above
        GPIO.output(redstsLed, 0) # Indicate SmartE has passed the tests
        sleep(3) # Sleep to indicate the result from above
        ...
        sleep(0.5) # Sleep to indicate the result from above 
        GPIO.output(redstsLed, 0) # Indicate SmartE has passed the tests
        sleep(0.5) # Sleep to indicate the result from above
